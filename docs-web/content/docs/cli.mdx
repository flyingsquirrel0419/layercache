...
Running `invalidate` without `--pattern` or `--tag` defaults to matching all keys (`*`). Explicit `--pattern "*"` is also treated as a full-cache invalidation. Both forms require `--force` to prevent accidental cache wipes:
...
Bypass the safety guard for full-cache `invalidate` operations. Required when running `invalidate` without `--pattern` or `--tag`, and when using explicit `--pattern "*"`.
