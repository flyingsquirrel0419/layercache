...
<PackageManagerTabs command="layercache stats --redis redis://localhost:6379" exec={true} />
...
Running `invalidate` without `--pattern` or `--tag` defaults to matching all keys (`*`). Every explicit pattern made only from `*` and `?`, including `*`, `**`, and `?*`, is also treated as a full-cache invalidation. These forms require `--force` to prevent accidental cache wipes:
...
layercache invalidate --redis redis://localhost:6379 --pattern "*" --force
layercache invalidate --redis redis://localhost:6379 --pattern "?*" --force
```
...
Bypass the safety guard for full-cache `invalidate` operations. Required when running `invalidate` without `--pattern` or `--tag`, and when using a wildcard-only pattern made from `*` and `?`.
