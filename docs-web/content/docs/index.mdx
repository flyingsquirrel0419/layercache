...
Layercache 4.1 hardens cache isolation and distributed invalidation:
...
- `createTrpcCacheMiddleware` and `cacheGraphqlResolver` require a `keyResolver` and drop `allowImplicitContextCaching`, preventing caller-specific data from being cached under a shared key.
...
Read the [migration guide](./migration#upgrading-to-41) before upgrading an existing deployment.
