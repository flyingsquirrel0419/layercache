...
- `allowPrivateCaching` - Allow implicit URL-based keys (default: false). Requests with common sensitive query parameters such as `access_token`, `api_key`, `apikey`, `auth`, `authorization`, `code`, `credentials`, `id_token`, `jwt`, `password`, `private_key`, `refresh_token`, `secret`, `session`, `sessionid`, `session_id`, and `token` bypass implicit caching unless you provide a custom `keyResolver`.
