...
const diskLayer = new DiskLayer({
  ...
  allowLegacyPlaintext: false                      // Default: reject plaintext when protection is enabled
})
```
...
When protection is configured, plaintext legacy entries are rejected by default. Use `allowLegacyPlaintext: true` only during a controlled migration window:
